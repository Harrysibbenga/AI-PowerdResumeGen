...
async def generate_resume_with_deepseek(profile_data: Dict[str, Any], tone: str = "professional") -> Dict[str, Any]:
    """
    Generate resume content using DeepSeek AI (open-source alternative to GPT)
    ...
    # We'll use the same prompt structure as the GPT service
    system_prompt = _create_system_prompt(profile_data.get("industry", "general"), tone)
    
    try:
        # This is a placeholder for actual DeepSeek API integration
        # Modify this to match the actual DeepSeek API when implemented
        async with httpx.AsyncClient() as client:
            ...
            # For MVP implementation, since DeepSeek API may differ:
            # This is a placeholder response handling
            if response.status_code != 200:
                # Fallback to GPT for now if this fails
                from app.services.gpt_service import generate_resume_with_gpt
                return await generate_resume_with_gpt(profile_data, tone)
        ...
        # For MVP, fallback to GPT if DeepSeek fails
        try:
            from app.services.gpt_service import generate_resume_with_gpt
            return await generate_resume_with_gpt(profile_data, tone)
        except:
            raise Exception(f"Error generating resume with DeepSeek: {str(e)}")
